...
export function AlerteFinSeance({ alertes, onRetourSeance, onTerminer }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-black/70">
      <div
        className="flex max-h-[85vh] w-full max-w-md flex-col rounded-t-3xl bg-white p-4"
        style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 1rem)' }}
      >
        <h2 className="mb-3 text-lg font-semibold text-slate-900">Séries non validées</h2>
        <div className="flex-1 overflow-y-auto">
          {restantes.map((a) => (
            <div key={a.groupe} className="mb-3 rounded-2xl border border-amber-300 bg-amber-50 p-3">
              ...
              <div className="mt-2 flex gap-2">
                <button
                  ...
                  className="min-h-11 flex-1 rounded-xl border border-amber-300 text-sm text-amber-700"
                >
                  Ajouter une série
                </button>
                <button
                  ...
                  className="min-h-11 flex-1 rounded-xl bg-amber-800 text-sm text-slate-800"
                >
                  C'est voulu
                </button>
              </div>
          ...
          {restantes.length === 0 && (
            <p className="text-sm text-slate-500">Tout est pris en compte, tu peux terminer.</p>
          )}
        </div>
        <button
          type="button"
          ...
          className="mt-3 min-h-14 rounded-2xl bg-accent font-semibold text-slate-950 disabled:opacity-40"
        >
          Terminer la séance
        </button>
      </div>
